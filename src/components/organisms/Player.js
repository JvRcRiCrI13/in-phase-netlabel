    ...
    return (
        <div className="fixed bottom-0 z-50 w-full border-t border-white/10 bg-black/60 backdrop-blur-xl transition-all duration-500 shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
            ...
            <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 relative">
                ...
                <div className="relative w-full h-full flex md:hidden items-center">
                    ...
                    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-6 z-20">
                        <button
                            ...
                            className={`text-gray-400 transition-all active:text-white active:scale-95 ${(!hasPrevious && (!audioRef.current || audioRef.current.currentTime <= 2)) ? 'opacity-30' : 'opacity-100'}`}
                        >
                        ...
                        <button
                            onClick={togglePlay}
                            className="flex-shrink-0 flex h-12 w-12 items-center justify-center rounded-full bg-white text-black transition-all active:scale-90 shadow-[0_0_15px_rgba(255,255,255,0.2)]"
                        >
                            {isPlaying ? (
                        ...
                        <button
                            onClick={playNext}
                            disabled={!hasNext}
                            className={`text-gray-400 transition-all active:text-white active:scale-95 ${!hasNext ? 'opacity-30' : 'opacity-100'}`}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8">
                ...
                <div className="hidden md:flex items-center justify-center w-1/3 gap-8">
                    <button onClick={handlePrevious} disabled={!hasPrevious && (!audioRef.current || audioRef.current.currentTime <= 2)} className={`text-gray-400 transition-all hover:text-white hover:scale-110 ${(!hasPrevious && (!audioRef.current || audioRef.current.currentTime <= 2)) ? 'opacity-30 cursor-not-allowed' : 'opacity-100'}`}>
                    ...
                    <button onClick={togglePlay} className="flex h-12 w-12 items-center justify-center rounded-full bg-white text-black transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_30px_rgba(34,211,238,0.5)]">
                        {isPlaying ? <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" className="w-6 h-6"><path fillRule="evenodd" d="M6.75 5.25a.75.75 0 01.75-.75H9a.75.75 0 01.75.75v13.5a.75.75 0 01-.75.75H7.5a.75.75 0 01-.75-.75V5.25zm7.5 0A.75.75 0 0115 4.5h1.5a.75.75 0 01.75.75v13.5a.75.75 0 01-.75.75H15a.75.75 0 01-.75-.75V5.25z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 ml-1"><path fillRule="evenodd" d="M4.5 5.653c0-1.426 1.529-2.33 2.779-1.643l11.54 6.348c1.295.712 1.295 2.573 0 3.285L7.28 19.991c-1.25.687-2.779-.217-2.779-1.643V5.653Z" clipRule="evenodd" /></svg>}
                    </button>
                    <button onClick={playNext} disabled={!hasNext} className={`text-gray-400 transition-all hover:text-white hover:scale-110 ${!hasNext ? 'opacity-30 cursor-not-allowed' : 'opacity-100'}`}>
                ...
                <div className="hidden md:flex w-1/3 items-center justify-end gap-3 pl-4">
                    ...
                    <div ref={progressBarRef} onClick={handleSeek} className="h-1.5 w-32 lg:w-48 overflow-hidden rounded-full bg-white/10 cursor-pointer group hover:h-2 transition-all">
                        <div className="h-full bg-brand-cyan shadow-[0_0_15px_rgba(34,211,238,0.6)] relative" style={{ width: `${(progress / duration) * 100}%` }}>
